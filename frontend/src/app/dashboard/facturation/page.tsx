...
export default function FacturationPage() {
  ...
  const handleUploadBillPhoto = async (e: any) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        showNotification(`${t('facturation', 'notifBillImported')} (${data.bill.month})`);
        fetchBills();
      }
    } catch (err) {
      showNotification(t('facturation', 'notifUploadError'));
    } finally {
  ...
  const handleGenerateForecast = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        showNotification(`${t('facturation', 'notifForecastGenerated')} ${data.month}.`);
        fetchBills();
      }
    } catch (err) {
      showNotification(t('facturation', 'notifForecastError'));
    } finally {
  ...
  const handleConfirmActual = async (billId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.error) {
        showNotification(t('facturation', 'notifActualSaved'));
        setConfirmingBillId(null);
        fetchBills();
      }
    } catch (err) {
      showNotification(t('facturation', 'notifActualError'));
    }
  };
  ...
  const handleAddManualBill = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.error) {
        showNotification(t('facturation', 'notifBillAdded'));
        setManualMonth(''); setManualAmount(''); setManualBillMode(false);
        fetchBills();
      }
    } catch (err) {
      showNotification(t('facturation', 'notifAddError'));
    }
  };
  ...
  const downloadAudit = () => {
    ...
    showNotification(t('facturation', 'notifAuditDownloaded'));
  };
  ...
  const downloadInvoice = (inv: any) => {
    ...
    showNotification(`${inv.id} — ${t('facturation', 'notifInvoiceDownloaded')}`);
  };
    ...
    <div>
      <div style={{ marginBottom: '32px' }}>
        <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '8px', letterSpacing: '0.05em' }}>
          <span style={{ color: 'var(--primary)' }}>{t('nav', 'facturation')}</span> / {t('facturation', 'breadcrumb')}
        </div>
        <h1 className="text-gradient" style={{ fontSize: '28px', fontWeight: 800, marginBottom: '8px' }}>{t('facturation', 'title')}</h1>
      ...
      <div className="facturation-grid">
        ...
        <div className="facturation-left-col">
          ...
          <div className="glass-card glow-card" style={{ padding: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
              <div>
                <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-muted)', marginBottom: '8px' }}>{t('facturation', 'verifiedSavings')}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderTop: '1px solid var(--surface-border)', paddingTop: '24px', flexWrap: 'wrap', gap: '16px' }}>
              <div style={{ display: 'flex', gap: '48px', flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>{t('facturation', 'grossSavings')}</div>
                  <div style={{ fontSize: '16px', fontWeight: 700 }}>{grossSavings.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} FCFA</div>
                </div>
                <div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>{t('facturation', 'gainShareLabel')}</div>
              ...
              <button onClick={downloadAudit} className="btn-primary" style={{ width: 'auto', display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 24px', border: 'none', cursor: 'pointer' }}>
                <Download size={16} /> {t('facturation', 'downloadAudit')}
              </button>
            </div>
          </div>
          ...
          <div className="glass-card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                <h3 style={{ fontSize: '14px', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{t('facturation', 'auditTrail')}</h3>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--primary)', fontSize: '12px', fontWeight: 700 }}>
                <CheckCircle size={14} /> {t('facturation', 'tamperProof')}
              </div>
            </div>
            ...
            <table className="audit-table">
              <thead>
                <tr>
                  <th>{t('facturation', 'colTimestamp')}</th>
                  <th>{t('facturation', 'colAction')}</th>
                  <th>{t('facturation', 'colRef')}</th>
                  <th>{t('facturation', 'colStatus')}</th>
                </tr>
              </thead>
              <tbody>
                ...
                {auditTrail.length === 0 && !loading && (
                  <tr>
                    <td colSpan={4} style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>{t('facturation', 'noRecentAudit')}</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div style={{ textAlign: 'center', marginTop: '24px' }}>
              <a href="#" onClick={(e) => { e.preventDefault(); showNotification(t('facturation', 'ledgerSoon')); }} style={{ color: 'var(--primary)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', textDecoration: 'none' }}>{t('facturation', 'viewFullLedger')}</a>
            </div>
          </div>
          ...
          <div className="glass-card">
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '20px' }}>
              ...
              <button
                ...
                {uploadingBill ? t('facturation', 'analyzing') : t('facturation', 'uploadBill')}
              </button>
              <button
                ...
                {t('facturation', 'generateForecast')}
              </button>
            </div>
            ...
            {!manualBillMode ? (
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '20px', padding: '14px', border: '1px solid var(--surface-border)', borderRadius: '8px' }}>
                <input className="input-field" placeholder={t('facturation', 'monthPlaceholder')} value={manualMonth} onChange={(e) => setManualMonth(e.target.value)} style={{ width: '140px' }} />
                <input className="input-field" type="number" placeholder={t('facturation', 'amountPlaceholder')} value={manualAmount} onChange={(e) => setManualAmount(e.target.value)} style={{ width: '160px' }} />
                <button onClick={handleAddManualBill} className="btn-primary" style={{ width: 'auto', padding: '8px 16px', border: 'none', cursor: 'pointer' }}>{t('common', 'add')}</button>
                <span onClick={() => setManualBillMode(false)} style={{ fontSize: '12px', color: 'var(--text-muted)', cursor: 'pointer' }}>{t('common', 'cancel')}</span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {bills.map((bill) => {
                const sourceBadge = bill.is_forecast
                  ? { label: t('facturation', 'badgeForecast'), color: '#F59E0B' }
                  : bill.source === 'ocr'
                  ? { label: t('facturation', 'badgePhoto'), color: 'var(--primary)' }
                  : { label: t('facturation', 'badgeManual'), color: 'var(--text-muted)' };
                  ...
                  <div key={bill.id} style={{ border: `1px solid ${needsActual ? 'rgba(245,158,11,0.35)' : 'var(--surface-border)'}`, borderRadius: '8px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    {ecartPct !== null && (
                      <div style={{ fontSize: '11px', color: Math.abs(ecartPct) <= 10 ? 'var(--primary)' : '#F59E0B', fontWeight: 600 }}>
                        {t('facturation', 'actualLabel')} : {bill.actual_amount_xof.toLocaleString('fr-FR')} FCFA ({ecartPct > 0 ? '+' : ''}{ecartPct}% {t('facturation', 'vsForecast')})
                      </div>
                    )}
                    ...
                    {needsActual && (
                      confirmingBillId === bill.id ? (
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                          ...
                          />
                          <button onClick={() => handleConfirmActual(bill.id)} className="btn-primary" style={{ width: 'auto', padding: '6px 14px', fontSize: '12px', border: 'none', cursor: 'pointer' }}>{t('facturation', 'validate')}</button>
                          <span onClick={() => setConfirmingBillId(null)} style={{ fontSize: '11px', color: 'var(--text-muted)', cursor: 'pointer' }}>{t('common', 'cancel')}</span>
                        </div>
                      ) : (
        ...
        <div className="facturation-right-col">
          ...
          <div className="glass-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, marginBottom: '24px', textTransform: 'uppercase' }}>{t('facturation', 'automatedSettlement')}</h3>
            ...
            <div style={{ border: '1px solid var(--primary-light)', backgroundColor: 'var(--primary-dim)', borderRadius: '8px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--primary)', fontWeight: 700, letterSpacing: '0.05em' }}>{t('facturation', 'connected')}</div>
                </div>
              </div>
            ...
            <div style={{ border: '1px solid var(--surface-border)', borderRadius: '8px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '0.05em' }}>{t('facturation', 'backupConfig')}</div>
                </div>
              </div>
              <span onClick={() => showNotification(t('facturation', 'paymentSoon'))} style={{ fontSize: '12px', color: 'var(--primary)', fontWeight: 600, cursor: 'pointer' }}>{t('facturation', 'change')}</span>
            </div>

            <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.05em', marginBottom: '8px', textTransform: 'uppercase' }}>{t('facturation', 'nextCycle')} : 01 OCT 2026</div>
          ...
          {/* Commission History */}
          <div className="glass-card">
            <h3 style={{ fontSize: '14px', fontWeight: 700, marginBottom: '24px', textTransform: 'uppercase' }}>{t('facturation', 'commissionHistory')}</h3>
              ...
              {invoices.map((inv) => (
                <div key={inv.id} style={{ border: '1px solid var(--surface-border)', borderRadius: '8px', padding: '16px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div onClick={() => downloadInvoice(inv)} style={{ fontSize: '10px', color: 'var(--primary)', fontWeight: 700, cursor: 'pointer' }}>{t('facturation', 'download')}</div>
                  </div>
                </div>
              ))}
              {invoices.length === 0 && !loading && (
                <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>{t('facturation', 'noInvoices')}</div>
              )}
            </div>

            {/* Model Distribution */}
            <div style={{ backgroundColor: 'rgba(230,244,234,0.3)', borderRadius: '8px', padding: '16px', border: '1px solid var(--primary-light)' }}>
              <h4 style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '16px' }}>{t('facturation', 'modelSplit')}</h4>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    ...
                    <div style={{ fontSize: '12px', fontWeight: 600 }}>{t('facturation', 'retainedSavings')}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: 'var(--text-muted)' }}></div>
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{t('facturation', 'commissionSplit')}</div>
                  </div>
                </div>
              </div>
            </div>
